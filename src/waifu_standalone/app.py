...
class WaifuService:
    ...
    def handle_event(self, event: InboundEvent) -> OutboundMessage | None:
        ...
        latest_message = text or event.to_memory_text()
    ...
    def _archive_if_needed(self, launcher_id: str, launcher_type: str, *, assistant_name: str) -> None:
        if not self.config.summarization_mode:
            return
        self.memory.maybe_archive_history(
            launcher_id,
            launcher_type,
    ...
    def _should_reply(self, event: InboundEvent) -> bool:
        if event.launcher_type != "group":
            return True
        if not self.config.bot_account_id:
            return True
    ...
    def save_other_panel(self, payload: dict[str, object]) -> dict[str, object]:
        ...
        self.config.group_follow_up_window_seconds = float(payload.get("group_follow_up_window_seconds", self.config.group_follow_up_window_seconds) or self.config.group_follow_up_window_seconds)
