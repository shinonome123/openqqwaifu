...
class LegacyMigrator:
    ...
    def migrate_session_state(self) -> None:
        sessions = self._service.session_repository.list_sessions()
        if not sessions:
            return
        active_character = self._service._active_character_id()
        existing_keys = {
            self.knowledge_entry_key(entry)
            for entry in self._service.knowledge_repository.list_knowledge(
                limit=max(1, self._service.knowledge_repository.knowledge_count(character_id=active_character)),
        ...
        for session in sessions:
            ...
            if legacy_members:
                for payload in self.iter_legacy_group_members(legacy_members):
                    ...
                    if preferred_name or payload.get("profile_summary") or payload.get("onboarding_status"):
                        ...
                        self._service.member_repository.save_member(member_payload)
            ...
            if isinstance(legacy_long_term, list):
                for item in legacy_long_term:
                    ...
                    if key in existing_keys:
                        continue
                    self._service.knowledge_repository.add_knowledge(
                        character_id=str(session.character_id or self._service._active_character_id()).strip(),
            ...
            if changed:
                self._service.session_repository.save(session)
    ...
    def repair_character_isolation(self, character_id: str = "") -> None:
        ...
        if not target_ids:
            return
        for session in self._service.session_repository.list_sessions():
        ...
        for session_character in target_ids:
            for member in self._service.member_repository.list_members(limit=5000, character_id=session_character):
