...
from .onebot_actions import OneBotActionClient
from .onebot_ws import OneBotWsActionClient
...
class NoticeDispatcher:
    ...
    def sync_group_members(self, group_id: str) -> dict[str, object]:
        ...
        for item in members:
            ...
            saved = svc.member_repository.save_member(
                {
            ...
            synced.append(saved)
        missing_count = svc.member_repository.mark_group_members_missing(
            group_id=safe_group_id,
    ...
    async def async_group_members(self, group_id: str) -> dict[str, object]:
        ...
        for item in members:
            ...
            saved = await asyncio.to_thread(
                svc.member_repository.save_member,
                {
                    "group_id": safe_group_id,
                    "user_id": user_id,
                    "qq_nickname": str(item.get("nickname", "") or ""),
            ...
            synced.append(saved)
        missing_count = await asyncio.to_thread(
            svc.member_repository.mark_group_members_missing,
            group_id=safe_group_id,
            active_user_ids=active_user_ids,
            membership_status="left",
            last_sync_at=synced_at,
    ...
    def _handle_group_increase_notice(self, payload: dict[str, object]) -> dict[str, object]:
        ...
        if svc.config.member_auto_sync:
            try:
                ...
                pass
        saved = svc.member_repository.save_member(member_payload)
    ...
    async def _ahandle_group_increase_notice(self, payload: dict[str, object]) -> dict[str, object]:
        ...
        if svc.config.member_auto_sync:
            try:
                ...
                pass
        saved = await asyncio.to_thread(svc.member_repository.save_member, member_payload)
        return {"status": "ok", "reason": "member_joined", "member": saved}
    ...
    def _handle_group_decrease_notice(self, payload: dict[str, object]) -> dict[str, object]:
        ...
        if bot_id and user_id == bot_id:
            affected = svc.member_repository.mark_group_members_missing(
                group_id=group_id,
                active_user_ids=[],
        ...
        saved = svc.member_repository.mark_member_membership(
            group_id=group_id,
            user_id=user_id,
    ...
    async def _ahandle_group_decrease_notice(self, payload: dict[str, object]) -> dict[str, object]:
        ...
        status = "left" if sub_type == "leave" else "removed"
        saved = await asyncio.to_thread(
            svc.member_repository.mark_member_membership,
            group_id=group_id,
            user_id=user_id,
            membership_status=status,
            last_sync_at=synced_at,
