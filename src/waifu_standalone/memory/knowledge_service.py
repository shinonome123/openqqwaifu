...
class KnowledgeService:
    ...
    def _persist_extracted_knowledge(
        ...
        *,
        ...
        return svc.knowledge_repository.save_knowledge(payload)
    ...
    def _existing_knowledge_entry(
        ...
        limit = max(
            80,
            svc.knowledge_repository.knowledge_count_for_active(),
        )
        target = " ".join(str(summary or "").split()).strip().casefold()
        for item in svc.knowledge_repository.list_knowledge_for_active(limit=limit):
    ...
    def _member_record(self, event: InboundEvent) -> dict[str, Any] | None:
        svc = self.service
        return svc.member_repository.get_member_for_active(
    ...
    def _session_knowledge_entries(self, event: InboundEvent, query: str) -> list[dict[str, Any]]:
        ...
        entries = svc.knowledge_repository.list_knowledge_for_active(limit=max(80, svc.config.memory_graph_limit * 6))
    ...
    def _detail_knowledge_entries(self, session: SessionMemory) -> list[dict[str, Any]]:
        svc = self.service
        entries = svc.knowledge_repository.list_knowledge(
    ...
    def _knowledge_count_for_session(self, session: SessionMemory) -> int:
        ...
        count = svc.knowledge_repository.count_knowledge_for_scopes(
            scopes,
            character_id=current_character,
        )
        if count:
            return count
        total = max(1, svc.knowledge_repository.knowledge_count(character_id=current_character))
        entries = svc.knowledge_repository.list_knowledge(limit=total, character_id=current_character)
        if session.launcher_type == "group":
